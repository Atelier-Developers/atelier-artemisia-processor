...
class ControlUnit:
    ...
    def build(self):
        ...
        and_addi = And((not_opcode5, not_opcode4, self.opcode[3], not_opcode2, not_opcode1, not_opcode0),
                       f"{self.name}_and2_alusrc")
        ...
        reg_dst = and0
        alu_src = Or((And((self.opcode[5], not_opcode4, not_opcode2, self.opcode[1], self.opcode[0]),
                          f"{self.name}_and1_alusrc"),
                      and_addi), f"{self.name}_or_alusrc")
        mem_to_reg = And((self.opcode[5], not_opcode4, not_opcode3, not_opcode2, self.opcode[1], self.opcode[0]))
        and1 = And((self.opcode[5], not_opcode4, not_opcode3, not_opcode2, self.opcode[1], self.opcode[0]),
                   f"{self.name}_and1")
        reg_write = Or((and0, and1, and_addi))
        mem_read = and1
